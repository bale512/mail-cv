...
# 解析VOC2007,生成json数据返回给前端
def voc_convert_to_jason(file_path):
    if os.path.exists(file_path):
        result = []
        dom = xml.parse(file_path)
        # 得到文档元素对象
        root = dom.documentElement
        boxes = root.getElementsByTagName('object')
        for box in boxes:
            result_boxes = dict()
            result_boxes['name'] = box.getElementsByTagName('name')[0].childNodes[0].data
            postion = box.getElementsByTagName('bndbox')[0]
            result_boxes['xmin'] = postion.getElementsByTagName('xmin')[0].childNodes[0].data
            result_boxes['ymin'] = postion.getElementsByTagName('ymin')[0].childNodes[0].data
            result_boxes['xmax'] = postion.getElementsByTagName('xmax')[0].childNodes[0].data
            result_boxes['ymax'] = postion.getElementsByTagName('ymax')[0].childNodes[0].data
            result.append(result_boxes)
        return result
    else:
        result = dict()
        result['code'] = '500'
        result['message'] = 'not found VOC'
        return result
