...
# 读取样本之前的标注信息
@app.route('/api/annotation/sampleLabel', methods=['GET'])
def get_sample_labels():
    if 'index' in request.args:
        annotation_name = request.args['index'] + '.xml'
        file_path = os.path.join(sys_config.ANNOTATIONS_FILE_PATH, annotation_name)
        return jsonify(tool.voc_convert_to_jason(file_path))
